...
>**Important**: The OpenAI Client is now configured to use Responses API from OpenAI: `https://api.openai.com/v1/responses`. <br>
>If you have set the `OPENAI_URL` in the .env file, either change the url accordingly or remove the variable from environment file to use the predefined configuration.
